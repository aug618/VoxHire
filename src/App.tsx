...
import { BarChart3, ChevronRight, FileText, History, Mic, Play, RotateCcw, ShieldCheck, Sparkles, Upload, Volume2 } from "lucide-react";
import { demoQuestions, demoReport, demoTranscript } from "./demo";
import { createSession, extractPdf, generateReport, mirrorTranscript } from "./services/api";
...
export default function App() {
  ...
  const progress = useMemo(() => Math.min(5, entries.filter((item) => item.role === "user").length), [entries]);
  ...
  function reset() {
    gateway.current?.close();
    gateway.current = null;
    setEntries([]); setReport(null); setSession(null); setDemoIndex(0); setNotice(""); setView("setup"); setGatewayStatus("ready");
  }
  ...
  return (
    <main className="app-shell">
      ...
      {view === "setup" && <SetupView mode={mode} setMode={setMode} setup={setup} setSetup={setSetup} onPdf={onPdfChange} onStart={startInterview} loading={isLoading} history={history} onOpenHistory={(item) => { setReport(item.report); setEntries(item.transcript); setView("report"); }} />}
      {view === "interview" && <InterviewView mode={mode} setup={setup} session={session!} entries={entries} progress={progress} status={gatewayStatus} demoIndex={demoIndex} loading={isLoading} onHold={(held) => gateway.current?.setCapturing(held)} onDemoAdvance={advanceDemo} onFinish={finishInterview} />}
      {view === "report" && report && <ReportView report={report} setup={setup} onRestart={reset} />}
    </main>
  );
}

function SetupView({ mode, setMode, setup, setSetup, onPdf, onStart, loading, history, onOpenHistory }: { mode: AppMode; setMode: (value: AppMode) => void; setup: SetupData; setSetup: (value: SetupData) => void; onPdf: (file?: File) => void; onStart: () => void; loading: boolean; history: StoredInterview[]; onOpenHistory: (item: StoredInterview) => void }) {
...
function InterviewView({ mode, setup, session, entries, progress, status, demoIndex, loading, onHold, onDemoAdvance, onFinish }: { mode: AppMode; setup: SetupData; session: SessionCreated; entries: TranscriptEntry[]; progress: number; status: "ready" | "listening" | "thinking" | "speaking" | "error"; demoIndex: number; loading: boolean; onHold: (value: boolean) => void; onDemoAdvance: () => void; onFinish: () => void }) {
  ...
  return <section className="interview-layout">
    ...
    <div className="interview-stage"><div className="stage-top"><div><p className="eyebrow">VoxHire 面试官</p><h1>林知远</h1></div><span className={`status status-${status}`}><i />{statusCopy}</span></div><div className="avatar-stage"><InterviewerAvatar status={status} /></div><div className="question-panel"><span>当前问题</span><p>{question}</p></div>
      {mode === "demo" ? <div className="voice-control demo-control"><button className="voice-button" onClick={onDemoAdvance} disabled={demoIndex >= 5}><Play size={25} /></button><div><strong>{demoIndex >= 5 ? "样例回答已完成" : "提交示例回答"}</strong><span>演示模式使用内置转写内容</span></div></div> : <div className="voice-control"><button className="voice-button" onPointerDown={() => onHold(true)} onPointerUp={() => onHold(false)} onPointerLeave={() => onHold(false)}><Mic size={25} /></button><div><strong>按住说话，松开结束录音</strong><span>静音后由本地语音服务确认回答结束</span></div></div>}
    </div>
